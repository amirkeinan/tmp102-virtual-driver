...
void sim_set_ambient_temperature(tmp102_virtual_hw_t *hw, float temp_celsius)
{
    ...
    /*
     * Calculate 12-bit / 13-bit digital count based on 0.0625°C resolution per LSB.
     * Round symmetrically to nearest integer to avoid precision truncation issues.
     */
    float counts_f = temp_celsius / TMP102_TEMP_RESOLUTION;
    int16_t counts = (counts_f >= 0.0f) ? (int16_t)(counts_f + 0.5f) : (int16_t)(counts_f - 0.5f);

    /*
     * Encode count into 16-bit register format according to Extended Mode (EM) bit:
     * - Normal Mode (EM = 0): 12-bit two's complement, left-justified (bits 15..4), bits 3..0 = 0.
     * - Extended Mode (EM = 1): 13-bit two's complement, left-justified (bits 15..3), bits 2..0 = 0.
     */
